/* ═══════════════════════════════════════════════════════════════
   ...
   PRESENCE (separate system, same boundary): the Railway Gateway
   bridge is read through lib/discord-presence.js with its own short
   TTL — OAuth identity and live status fail independently, so one
   outage can never take down the other.
...
import { getPresence as getPresenceFromBridge } from "./discord-presence.js";
...
/* Live presence via the Railway Gateway bridge (see
   discord-presence-bridge/). Independent of OAuth on purpose: a
   revoked token must not kill the status dot, and a dead bridge
   must not kill the identity. Never throws. */
export async function getDiscordPresence() {
  return getPresenceFromBridge();
}
