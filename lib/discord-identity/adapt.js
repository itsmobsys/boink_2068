...
// Skill hover-accent grouping. The old site tinted only each chip's
// dot with its own color; the new UI varies hover hue by group, so
// map the existing skill names onto its four groups by keyword.
function skillGroup(name) {
  const n = String(name).toLowerCase();
  if (/(^|\s)ai(\s|$)|agents|openrouter|ollama/.test(n)) return "ai";
  if (/next|react|javascript|typescript|html|css/.test(n)) return "frontend";
  if (/node|python|flask|playwright|discord|rest apis|sql|redis|postgres|sqlite/.test(n))
    return "runtime";
  return "infra";
}
...
// Source pane lines derived from the same vibe.data object, so the
// new Source specimen shows the owner's real file, not placeholder
// copy. Arrays collapse to one line — the pane is a specimen, the
// full file already lived in the old UI.
